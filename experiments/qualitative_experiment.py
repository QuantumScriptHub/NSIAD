...
seqL = 31
kf = NSIAD(signals,window_length=seqL, input_range=2*seqL)
...
plt.plot(T[1000:1300],rec_x[1000:1300],linestyle='-.',label='NSIAD-RECON')
plt.plot(T[1000:1300],pred_x[1000:1300],linestyle=':',label='NSIAD-PRED')
plt.plot(T[1000:1300],mu_x[1000:1300],linestyle='--',label='NSIAD mean')
plt.fill_between(T[1000:1300], mu_x[1000:1300]-std_x[1000:1300], mu_x[1000:1300]+std_x[1000:1300], alpha=0.5,color='y',label='NSIAD one std interval')
...
for j in range(dim):
    ...
    if j == 0:
        ...
        plt.plot(T,recon_scores[1:],label='NSIAD-RECON',color=colors[j%7])
        plt.ylabel('residual error')
        ax.xaxis.set_ticklabels([])
    elif j == 2:
        plt.plot(T,pred_scores,label='NSIAD-PRED',color=colors[j%7])
        ax.yaxis.set_label_position("right")
        ax.yaxis.tick_right()
        plt.ylabel('residual error')
        ax.xaxis.set_ticklabels([])
    elif j == 3:
        plt.plot(T,z_scores,label='NSIAD',color=colors[j%7])
